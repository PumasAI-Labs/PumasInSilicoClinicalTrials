...
### Calibrate Virtual Population: MILP Solution
...
```{julia}
#| echo: false
classify_response(tumor_size::AbstractVector) = cut(tumor_size, [0.14, 0.7, 1.0]; labels = ["CR", "PR", "SD", "PD"], extend = true)
vpop.response = classify_response(vpop.tumor_size_final)
```
...
Calibrate the virtual population by maximizing the number of selected VPs subject to a bound on the
total variation (TV) distance between the selected and target distributions (see
[Tutorial 5](tb_05_milp_calibration_tutorial.qmd) for the full derivation):

```{julia}
# Maximum TV distribution error
epsilon = 0.05
...
@variable(model, δ[1:nrow(target_distribution)] >= 0)
...
for (δ_c, row) in zip(δ, eachrow(target_distribution))
    ...
    @constraint(model, δ_c >= N_c - p_c * N_total)
    @constraint(model, δ_c >= p_c * N_total - N_c)
end

# Upper bound on TV distribution error: TV ≤ ε
# Since sum(δ) = 2 · TV · N_total, this is equivalent to: sum(δ) ≤ 2ε · N_total
@constraint(model, sum(δ) <= 2 * epsilon * N_total)
...
specs_data = data(all_data) *
    mapping(
    :time => (t -> t / 7) => "Time (weeks)", :tumor_size => "Normalized Tumor Size",
...
```{julia}
#| output: false
final_classify = @transform final :response = classify_response(:tumor_size)
```
