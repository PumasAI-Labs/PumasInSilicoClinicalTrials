...
[HBV-04](hbv_04_copula_vpop_tutorial.qmd), then simulate 5 years of untreated chronic infection to
obtain each patient's baseline HBsAg at steady state.
...
Baseline HBsAg is the steady-state level reached
after 5 years of untreated chronic infection [@cortes2025HBV].
...
#| output: false
t_chronic = 5 * 365
sims = map(1:n_vps) do id
    subj = Subject(; id, time = [t_chronic], covariates = (; NA = false, IFN = false))
    randeffs = (; η = rand(correlated_randeffs_dist))
    sim = simobs(hbv_model, subj, hbv_params, randeffs; simulate_error = false)
    while !isvalid(sim)
        randeffs = (; η = rand(correlated_randeffs_dist))
        sim = simobs(hbv_model, subj, hbv_params, randeffs; simulate_error = false)
    end
...
vpop = postprocess(sims) do generated, _
    (; log10_HBsAg_baseline = only(generated.log10_HBsAg))
end |> DataFrame
...
for val in vpop.log10_HBsAg_baseline
...
for row in eachrow(everest_target)
    ...
    nc = sum(x[row.log10_lower .<= vpop.log10_HBsAg_baseline .< row.log10_upper])
...
for val in calibrated_vpop.log10_HBsAg_baseline
    for b in 1:nrow(everest_target)
        if everest_target[b, :log10_lower] <= val < everest_target[b, :log10_upper]
...
data_specs = data(df) * mapping(:log10_HBsAg_baseline, col = :stage => sorter("Original", "Calibrated")) * histogram(; bins = vcat(everest_target.log10_lower, everest_target.log10_upper[end]), normalization = :probability)
...
for (δ_c, row) in zip(δ, eachrow(everest_target))
    p_c = row.pct / 100
    N_c = sum(y[row.log10_lower .<= vpop.log10_HBsAg_baseline .< row.log10_upper])
