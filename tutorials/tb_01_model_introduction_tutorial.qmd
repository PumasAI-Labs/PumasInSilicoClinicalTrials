...
This tutorial introduces the Tumor Burden Model, a foundational example for understanding In Silico Clinical Trials (ISCTs).
The model describes tumor dynamics during cancer chemotherapy and will be used throughout this tutorial series [@cortes2025; @qi2022].
...
- `Random`: Julia standard library for random number generation (`randn`, `randn!`, `seed!`, etc.)

## Part 1: The Model Equation

### Mathematical Formulation

The tumor burden model describes the total tumor size $N(t)$ as the sum of two exponential terms [@qi2022]:
...
- `tvf ∈ RealDomain(lower=0.0, upper=1.0)`: Typical value of `f` (bounded between 0 and 1)
...
Nadir (minimum tumor size) occurs when $N'(t) = 0$, i.e., when
$$ -k f \exp{(-kt)} + g (1-f) \exp{(gt)} = 0. $$
...
4. **Pumas model structure:**
   ...
   - `@pre`: Parameter transformations (logit-normal for `f`, log-normal for `g` and `k`)
   - `@dynamics`: ODE system
