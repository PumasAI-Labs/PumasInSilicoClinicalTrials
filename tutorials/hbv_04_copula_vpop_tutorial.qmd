...
We convert the target Spearman correlation to the underlying Gaussian's (Pearson) correlation (see
[TB-04](tb_04_copula_vpop_tutorial.qmd) for the formula), so the copula reproduces it exactly:
...
By construction, the copula's Spearman correlation equals the target `Rho`:
...
The sample drawn from the copula reproduces the target Spearman correlation `Rho`,
up to sampling noise:

```{julia}
maximum(abs, corspearman(correlated_randeffs') - Rho)   # ≈ 0: sample Spearman vs target
```

Because the `@pre` transforms are strictly increasing, they leave the ranks unchanged,
so the parameters have the *exact* same Spearman correlation as the random effects:
...
The Pearson correlation, by contrast, is not preserved:

```{julia}
maximum(abs, cor(param_matrix) - cor(correlated_randeffs'))   # ≠ 0: Pearson not preserved
```

So the Spearman correlation of the parameters equals the target `Rho`, up to sampling noise,
