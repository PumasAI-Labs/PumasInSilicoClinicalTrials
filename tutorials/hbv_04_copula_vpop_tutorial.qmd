...
The correlation matrix from the NLME fit [@cortes2025HBV] captures the dependence structure among
the 8 correlated parameters.
...
```{julia}
# Correlation matrix (9×9) from @cortes2025HBV Figure S5B (Pearson correlation of the random effects)
Rho = [
...
```{julia}
spearman_target = corspearman(copula)
maximum(abs.(spearman_target - Rho))   # induced Spearman is close to Rho, but a little smaller
```
...
```{julia}
maximum(abs.(cor(param_matrix) - Rho))   # ≠ 0: Pearson correlation not preserved
```
...
for `p_S`–`k_A` is `{julia} round(spearman_target[1, 5], digits = 3)` — close to,
but smaller in magnitude than, the `{julia} Rho[1, 5]` we supplied (from @cortes2025HBV).
...
1. **The HBV model has 8 correlated parameters** whose NLME-estimated
   correlations must be preserved in the virtual population
