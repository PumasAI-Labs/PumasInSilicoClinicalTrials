...
# Convert a copula sample (length-9 vector) into a named tuple of random effects
# matching the order of the `SklarDist` marginals above.
function _sample_randeffs(dist)
    sample = rand(dist)
    return (;
        z_p_S = sample[1],
        z_β = sample[2],
...
baseline_vrandeffs = map(_ -> _sample_randeffs(correlated_randeffs_dist), baseline_pop)
...
while true
    ...
    map!(_ -> _sample_randeffs(correlated_randeffs_dist), vrandeffs, pop)
