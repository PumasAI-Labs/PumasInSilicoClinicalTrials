...
Parameters are parameterized on the **log10 scale**.
Inter-individual variability is expressed via **named random effects with explicit distributions**
in the `@random` block:

- `LogitNormal` for parameters whose log10 values are restricted to a physiologically plausible
  range (`p_S`, `β`, `m`, `ϵ_NA`, `ϵ_IFN`, `r_E_IFN`) — mapped back to that range in `@pre`.
- `Normal` for parameters with unbounded log10 values (`k_P`, `k_A`, `conv_E`) — exponentiated
  in `@pre`.

Individual values on the natural scale are reconstructed in `@pre` via monotonic transformations
(logistic rescaling or `exp10`), which preserves rank correlations in later copula sampling.
:::
...
hbv_estimated_params = (;
    ...
    # IIV standard deviations (on each parameter's transformed scale)
    ω_p_S = 0.632,
    ω_β = 2.489,
    ω_m = 0.827,
    ω_k_P = 1.292,
...
sim_no_tx = simobs(
    ...
    subject_no_tx,
    hbv_params,
    center_randeffs(hbv_model, subject_no_tx, hbv_params);
    obstimes = 0:7:(5 * 365),
...
sims = simobs(
    ...
    pop,
    hbv_params,
    center_randeffs(hbv_model, pop, hbv_params);
...
#| warning: false
# Simulate 50 patients with IIV sampled from the estimated random-effect distributions
sims = map(1:50) do id
    subj = Subject(;
        id,
        covariates_time = [0.0, t_chronic, t_na_suppressed, t_eot],
...
typical_randeffs = center_randeffs(hbv_model, typical_subject, hbv_params)
# Responder: favorable random effects
# For instance, higher immune enhancement via a large shift in the IFN boost r_E_IFN
# (three standard deviations above the typical logit location)
responder_subject = Subject(; id = "Responder", covariates_time, covariates)
responder_randeffs = merge(
    center_randeffs(hbv_model, responder_subject, hbv_params),
    (; z_r_E_IFN = logistic(logit(hbv_params.tv_log10_r_E_IFN / 4) + 3.0)),
)
