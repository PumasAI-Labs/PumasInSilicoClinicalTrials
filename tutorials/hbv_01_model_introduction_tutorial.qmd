...
Let's build the HBV model step by step.
...
println("HBV model defined successfully")
println("  State variables: 11")
println("  Fixed parameters: 22")
println("  Estimated parameters: 9")
```
...
pop_params = init_params(hbv_model)
...
println("Created subject for simulation")
```

### Simulate Natural History

```{julia}
# Simulate with zero random effects (typical patient)
Random.seed!(42)
...
println("Simulation complete: $(nrow(sim_df)) time points")
```
...
ag = draw!(fig[1, 1], plt, scales(Color = (; palette = [:red, :blue])))
