...
# Target Spearman correlation from the NLME estimates
...
Based on the target Spearman correlation matrix, we define a Gaussian copula.
We convert it to the underlying Gaussian's correlation (see
...
# Convert the target Spearman correlation to the underlying Gaussian's (Pearson) correlation
Rho_gauss = @. 2 * sin(π * Rho / 6)
copula = GaussianCopula(Rho_gauss)
```
