...
hbv_model = @model begin
    ...
    @reactions begin
        ...
        # ─── Innate-immunity-driven conversion of infected to refractory
        # (X = cytotoxic effect acts as catalyst)
        ρ * X, I --> R

        # ─── Cellular-immunity killing (E catalyst; produces debris D) ──
        # Full killing rate on infected cells, fractional (m/n) on target
        # and refractory cells.
        m * r_E_IFN * E, I --> D
        (m / n) * r_E_IFN * E, T --> D
        (m / n) * r_E_IFN * E, R --> D
        ...
        # ─── Virus production (from infected cells) and clearance ───────
        p_V * ϵ_NA * ϵ_IFN * I, 0 --> V
        d_V, V --> 0

        # ─── HBsAg production and clearance ─────────────────────────────
        p_S * I, 0 --> S
        d_V, S --> 0

        # ─── Dendritic-cell activation (Hill in I) ──────────────────────
        k_P * I / (ϕ_E + I), 0 --> P
        d_P, P --> 0

        # ─── Effector T-cell dynamics ───────────────────────────────────
        d_E * P, 0 --> E   # baseline generation from P
        r_E * E * P, 0 --> E   # P-stimulated expansion
        d_E_Q * Q^4 / (ϕ_Q^4 + Q^4), E --> 0   # Q-driven exhaustion
        d_E, E --> 0   # natural clearance

        # ─── Delayed immune signal Q (first-order lag of P) ─────────────
        d_Q * P, 0 --> Q
        d_Q, Q --> 0

        # ─── Cytotoxic effect X: logistic-bounded, HBsAg-inhibited ──────
        # X' = r_X·(1-X)·(I/(ϕ_E+I))·(1 - S_max·Hill) − d_X·X
        # HBsAg_ug_mL = c_HBsAg · (V+S) where c_HBsAg = 96·24000e6/6.022e23.
        # Inlined here because Catalyst-based @reactions cannot resolve @vars.
        # Split (1-X) into production + X-proportional decay sharing the modifier.
        r_X * (I / (ϕ_E + I)) *
            (
