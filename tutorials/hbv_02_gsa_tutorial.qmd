...
```{julia}
#| output: false
# Each named random effect is shifted by ±ω/3 on its native (logit or log10) scale.
# This matches what `(; η = ±sqrt.(diag(Ω))/3)` did before the refactor.
function _shifted_randeffs(sign)
    s = sign / 3
    return (;
        z_p_S = logistic(logit((hbv_params.tv_log10_p_S - 2) / 10) + s * hbv_params.ω_p_S),
        z_β = logistic(logit(hbv_params.tv_log10_β / 10 + 1) + s * hbv_params.ω_β),
        z_m = logistic(logit((hbv_params.tv_log10_m + 3) / 8) + s * hbv_params.ω_m),
        log10_k_P = hbv_params.tv_log10_k_P + s * hbv_params.ω_k_P,
        log10_k_A = hbv_params.tv_log10_k_A + s * hbv_params.ω_k_A,
        z_ϵ_NA = logistic(logit(hbv_params.tv_log10_ϵ_NA / 5 + 1) + s * hbv_params.ω_ϵ_NA),
        z_ϵ_IFN = logistic(logit(hbv_params.tv_log10_ϵ_IFN / 5 + 1) + s * hbv_params.ω_ϵ_IFN),
        z_r_E_IFN = logistic(logit(hbv_params.tv_log10_r_E_IFN / 4) + s * hbv_params.ω_r_E_IFN),
        log10_conv_E = hbv_params.tv_log10_conv_E + s * hbv_params.ω_conv_E,
    )
end
...
gsa_constantcoef = (
    ...
    :σ_add_log10_HBsAg, :σ_prop_log10_HBsAg, :σ_add_log10_HBV, :σ_prop_log10_HBV, :σ_prop_log10_ALT, :σ_prop_log10_CD8,
)
```
...
1. **Batch simulation** via `simobs` on a population enables efficient
   evaluation of the many model runs required for GSA

2. **The HBV QSP model has a richer sensitivity structure** than the simpler
   tumor burden model, with multiple potentially influential parameters

3. For comprehensive multi-output, multi-timepoint results,
